...
import argparse
# Create the parser
parser = argparse.ArgumentParser(description="Aorta Crop Detection for different factors.")

# Add an argument for the folder path
parser.add_argument('--src_folder_path', 
                    type=str, nargs='?',  
                    default = './VerteNet_predictions/clsa_f2_5000_detailed_outputs',  
                    help='Enter Source Folder.')
parser.add_argument('--dst_folder_path', 
                    type=str, nargs='?',  
                    default = './clsa_f2_5000_detailed_splines',  
                    help='Enter Destination Folder.')
parser.add_argument('--factors_range_min', 
                    type=float, nargs='?',  
                    default = 0.9,  
                    help='Enter the min value of factor range.')
parser.add_argument('--factors_range_max', 
                    type=float, nargs='?',  
                    default = 1.2,  
                    help='Enter the min value of factor range.')
...
data_folder = args.src_folder_path
data_folders = args.dst_folder_path
factor_range_min = args.factors_range_min
factor_range_max = args.factors_range_max
dpi = args.dpi

# data_folder = './VerteNet_predictions/clsa_30000_detailed_outputs'
landmarks_folder = './landmarks'


# fctr = 0
for fctr in tqdm(list(np.arange(factor_range_min, factor_range_max, 0.1))):
    fctr = round(fctr,1)
    dataset = {}
    sub_folders = list(os.walk(data_folder))[0][1]
    
    # data_folders = './clsa_30000'
    ...
    src_folder = os.path.join(data_folder, 'original_images') 
    ...
    for idx, img_name in enumerate(tqdm(fnames_combined)):
        ...
        #     # continue
            ...
            border_clip_flag, region_clip_flag = aorta_clip_detection_3.aorta_clip_detection(orig_img, ori_image_points, pts_detailed, img_name, FACTOR = fctr, check_region_clipping = True, spline_save_folder = factor_folder, dpi = dpi)
