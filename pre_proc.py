...
def generate_ground_truth(image,
    ...
    hm = np.zeros((1, image_h, image_w), dtype=np.float32)
    wh = np.zeros((6, 2*4), dtype=np.float32)
    reg = np.zeros((6, 2), dtype=np.float32)
    ind = np.zeros((6), dtype=np.int64)
    reg_mask = np.zeros((6), dtype=np.uint8)
    ...
    if pts_2.shape[0]!=24:
        ...
        print('ATTENTION!! image {} pts does not equal to 24!!! '.format(img_id))
